 ...
 */
export function createCorpusSupplement(options) {
    ...
    return {
        ...
        async get(params) {
            ...
            if (plane === undefined || id === undefined)
                return null;
            let presence;
            try {
                presence = resolvePresence(config, {
                ...
                : plane === "thought"
                    ...
                    : plane === "artifact"
                        ...
                        : (presence.namespaces.curatedReadScope.find((n) => n.endsWith(`/${plane}`)) ??
                            presence.namespaces.episodic);
            try {
                const obj = await client.getWithQuery(path, { namespace: ns }, { token: presence.token });
...
function splitLookup(lookup) {
    const idx = lookup.indexOf("/");
    if (idx <= 0 || idx === lookup.length - 1)
        return [];
    return [lookup.slice(0, idx), lookup.slice(idx + 1)];
}
function endpointForPlane(plane, id) {
    const safeId = encodeURIComponent(id);
    if (plane === "curated")
        return `/v1/curated/${safeId}`;
    if (plane === "concept")
        return `/v1/concepts/${safeId}`;
    if (plane === "episodic")
        return `/v1/episodic/${safeId}`;
    if (plane === "artifact")
        return `/v1/artifacts/${safeId}`;
    return undefined;
}
function toCorpusSearchResult(row) {
    return {
        corpus: CORPUS_NAME,
...
function provenanceLabelFor(plane) {
    if (plane === "curated")
        return "Curated knowledge (high provenance)";
    if (plane === "concept")
        return "Synthesized concept (system hypothesis)";
    if (plane === "episodic")
...
function toCorpusGetResult(obj, plane, lookup) {
